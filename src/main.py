...
def run_pipeline() -> None:
    """Run the Sprint 3 preprocessing, deduplication, and history pipeline."""
    ...
    saved_path = save_processed_opportunities(compared_opportunities, DEFAULT_OUTPUT_PATH)

    removed_duplicates = len(processed_opportunities) - len(deduplicated_opportunities)

    print("AI Opportunity Radar - Sprint 3 data pipeline")
    print(f"Processed {len(compared_opportunities)} opportunities.")
    print(f"Removed {removed_duplicates} duplicate opportunities.")
    print("Status counts:")

    for status, count in compared_opportunities["status"].value_counts().items():
